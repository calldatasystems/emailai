...
// inbox zero labels
function getemailaiLabelsKey({ emailAccountId }: { emailAccountId: string }) {
  return `labels:emailai:${emailAccountId}`;
}

export async function deleteemailaiLabels({
  emailAccountId,
}: {
  emailAccountId: string;
}) {
  const key = getemailaiLabelsKey({ emailAccountId });
  return redis.del(key);
}
