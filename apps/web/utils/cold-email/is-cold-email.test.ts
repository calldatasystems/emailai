...
describe("blockColdEmail", () => {
  ...
  it("should add cold email label when coldEmailBlocker is LABEL", async () => {
    vi.mocked(labelUtils.getOrCreateemailaiLabel).mockResolvedValue({
      id: "label123",
    });
    ...
    expect(labelUtils.getOrCreateemailaiLabel).toHaveBeenCalledWith({
      gmail: mockGmail,
      key: "cold_email",
    });
  ...
  it("should not modify labels when coldEmailBlocker is DISABLED", async () => {
    ...
    expect(labelUtils.getOrCreateemailaiLabel).not.toHaveBeenCalled();
