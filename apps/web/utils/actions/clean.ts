...
import { emailaiLabels } from "@/utils/label";
  ...
  .action(
    async ({
      ...
      const process = async () => {
        ...
        const query = `${daysOld ? `older_than:${daysOld}d ` : ""}-in:"${emailaiLabels.processed.name}"`;
      ...
      const markedDoneLabel = await getLabel({
        name:
          action === CleanAction.ARCHIVE
            ? emailaiLabels.archived.name
            : emailaiLabels.marked_read.name,
        gmail,
      });
...
export const changeKeepToDoneAction = actionClient
  ...
  .action(
    async ({ ctx: { emailAccountId }, parsedInput: { threadId, action } }) => {
      ...
      const actionLabel = await getOrCreateemailaiLabel({
        key: action === CleanAction.ARCHIVE ? "archived" : "marked_read",
        gmail,
      });
