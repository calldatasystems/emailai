...
import { publishArchive, type TinybirdEmailAction } from "@emailai/tinybird";
import {
  getRandomLabelColor,
  emailaiLabels,
  PARENT_LABEL,
...
export async function getOrCreateemailaiLabel({
  gmail,
  key,
}: {
  gmail: gmail_v1.Gmail;
  ...
  const { name, color, messageListVisibility } = emailaiLabels[key];
  const labels = await getLabels(gmail);
